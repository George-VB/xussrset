...
my($header_template, $footer_template, $section_template, $key_template, $key_separator_master, $key_separator_slave) = ("\# begin main block.*?\n", "\# end main block", "\#\n\#.*?\n\#\n", "\[\/\]\{0\,2\}\\s\*\[a\-z\_0\-9\]\+\?\\s\*\:", "\:", "\:");
...
sub parse_files($$) {
	...
	my($header_master, $footer_master, @sections_master) = (read_file($from_file));
	my($header_slave, $footer_slave, @sections_slave) = (read_file($to_file));
	my($sec_number, $result, $section, %keys, $key, @values) = (1, "");
	$result .= $header_slave;
	foreach $section (@sections_master) {
		%keys = %{$section};
	        $result .= join("", @{$keys{"-Section-Header-"}});
                print("Block #$sec_number\n");
		$sec_number++;
		foreach $key (sort {uc($a) cmp uc($b)} keys(%keys)) {
...
# look for slave-only values and add them
	print("Last block - unused strings\n");
	foreach $section (@sections_slave) {
		%keys = %{$section};
		foreach $key (sort {uc($a) cmp uc($b)} keys(%keys)) {
			if($key ne "-Section-Header-") {
				@values = get_key_values($key, $key_separator_master, $key_separator_slave, @sections_master);
				if(scalar(@values) == 0) {
...
# TODO not to rewrite file if it has not been changed
	...
	print TOFILE $result;
	close(TOFILE);
	if (rename ($to_file, $to_file . ".bak")) {
		print("$to_file renamed to $to_file\.bak\n");
		...
		print("$to_file can\'t be renamed. result saved to temp.tmp. ");
	}
}
