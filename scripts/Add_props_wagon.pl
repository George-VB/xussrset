...
sub PatchFile($) {
	...
	my($vehName, $vehCF, $vehRC, $vehSD, $vehWT, $vehTE, $vehPR, $vehCC, $vehLC) = ("pe2pe2pe2", 0, 0, 0, 0, 0, 0, 0, 0);
# find id
	$vehName = $1 if ($str =~ /item\s*\(FEAT_TRAINS\,\s*([0-9a-z_]+)\,\s*([0-9]+)\)/i);
	$serName = $vehName if (not defined($serName));
	$serName = $vehName if ($serName eq "");
# find CF
	$vehCF = $1 if ($str =~ /vehicle_dates\([0-9, ]+, ([0-9]+)\)/i);
# find RC
...
# replace rc
        ...
        $str =~ s/STORE_TEMP\([0-9]+\,\s*7\)\,\s*\/\/\s*Тара/STORE_TEMP\(PROP_${vehName}_WT\, 7\)\, \/\/ Тара/;
        $str =~ s/STORE_TEMP\(.*?\,\s*8\)\]\)\s*\/\/\s*Максимальная масса/STORE_TEMP\(PROP_${vehName}_WT + PROP_${vehName}_LC\, 8\)\]\) \/\/ Максимальная масса/;
# replace rc
